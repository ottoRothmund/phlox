...
export function SaveRepositoryButton({ repository }: { repository: Repository }) {
  ...
  return (
    <details className="save-menu relative">
      <summary
        ...
        className="button-secondary h-8 list-none cursor-pointer px-2.5 [&::-webkit-details-marker]:hidden"
      >
        <BookmarkSimple size={15} weight={saved ? "fill" : "regular"} />
        <span>{saved ? "Saved" : "Save"}</span>
      </summary>
      <div className="absolute right-0 top-10 z-30 w-64 max-w-[min(16rem,calc(100vw-2rem))] border border-border bg-surface p-1.5 shadow-[0_16px_40px_rgba(20,20,20,0.14)]">
